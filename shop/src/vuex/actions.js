...
// 请求获得商品详细信息
<<<<<<< HEAD
export const GoodsInfo = ({ commit }, data) => {
  commit('SET_LOAD_STATUS', true);
  return new Promise((resolve, reject) => {
    api.GoodsSel(data).then(res => {
      console.log(res.data.data.marketPrice);
      ...
      setTimeout(() => {
        const data = {
          ...
          promotion: ['跨店满减', '多买优惠'],
          remarksNum: res.data.data.appraiseNum,
          setMeal: [

            res.data.data.goodsspecs
...
export const goodsCats = ({ commit }, data) => {
  return new Promise((resolve, reject) => {
    api.setCats(data).then(res => {
...
=======
    ...
    api.register(data).then(res => {
      if (res.data.code === 1) {
        ...
        commit('SET_TOKEN_INDEX', res.data.data);
        resolve(true);
      } else {
        commit('SET_ERROR_IMG', res.data.message);
        resolve(false);
>>>>>>> gouqun1
      }
    });
...
=======
      if (res.data.code === 1) {
        localStorage.setItem('loginInfo', JSON.stringify(res.data.data));
        commit('SET_USER_LOGIN_INFO', res.data.data);
        resolve(true);
      } else {
        resolve(false);
      }
